...
$(".levelinfo-planeta").html("Planeta: Tierra");
$(".levelinfo-number").html("2");
$(".mision-info").html("Elimina a las naves enemigas, pero no elemines a tus aliados (naves de parte superior) [Cada nave viva te dara 100 puntos]");
// ------- JUEGO -------
...
function dibujarAliados() {
  for (var i in aliados) {
    ...
    ctx.fillRect(aliado.x, aliado.y, aliado.width, aliado.y);
  }
}
...
function moverNave() {
  if (teclado[37] && (misionstatus == "escondido" &&  operacionstatus=="escondido")) {
    nave.x -= 5;
    if (nave.x < 0) {
      nave.x = 0;
    }
  } else if (teclado[39] && (misionstatus == "escondido" &&  operacionstatus=="escondido")) {
...
function actualizarAliados() {
  ...
  for (var i in aliados) {
    ...
    if (aliado && aliado.estado == "vivo") {
      if (misionstatus == "escondido" &&  operacionstatus=="escondido") {
        aliado.contador++;
        aliado.x += Math.sin((aliado.contador * Math.PI + 90) / 90) * 5;
      }
...
function puntuacion() {
  if (nave.estado == "hit" || nave.estado == "vivo") {
    if(enemigos.length<10 || aliados.length<5){

      puntuaje = 100 * ((enemigosTotales - enemigos.length)+(2*aliados.length));
    }else{
    ...
    puntuaje = 100 * enemigosTotales +(2*aliadosTotales);
  }
}
